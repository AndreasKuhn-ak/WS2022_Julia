...
function update_sys_bias(Grid, Cell_list, Gridsize,Timesteps)
    ...
    mov_vec = [[0,-1],[1,0],[-1,0],[1,1],[-1,-1],[-1,1],[1,-1]]     #possible movement vectors to the next gridpoint
    sequence_vec = collect(1:length(mov_vec))                                           # sequence_vec is used to acess the mov_vec
